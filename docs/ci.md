...
| Release Readiness | `.github/workflows/release-readiness.yml` | `pull_request`/`push` to `main`, manual | A release-focused gate that adds formatting sanity checks, Android release compilation, Android database migration testing on an emulator, release-only debug-code exclusion checks, and stable Apple release verification without replacing the existing CI jobs. |
