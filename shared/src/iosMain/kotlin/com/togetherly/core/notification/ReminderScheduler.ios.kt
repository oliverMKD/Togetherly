 ...
 * ([reminderNotificationIdentifier]) — `addNotificationRequest` with an existing identifier
 * replaces the prior pending request for it, so re-scheduling (an edit, [ReminderScheduler.refresh])
...
private fun Result<Unit>.toReminderResult(diagnostics: OperationalDiagnostics): DataResult<Unit> = fold(
    ...
    onFailure = { throwable ->
        ...
        DataResult.Error(com.togetherly.core.error.AppError.Unexpected(throwable))
    },
)
